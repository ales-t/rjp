...
pub fn check_config(commands: &Vec<String>, length: usize) -> Result<(), RjpError> {
    if commands.len() < length {
        Err(RjpError::BadConfig(format!(
            "too few arguments, expected {}, got {}: {}",
            length, commands.len(), commands.join(" ")
        )))
    } else {
        Ok(())
    }
...
pub fn parse_mapping_def(mapping_str: &str) -> Result<FieldMapping, RjpError> {
    ...
    for field_pair in mapping_str.split(",") {
        ...
        if split.len() != 2 {
            return Err(RjpError::BadConfig(
                format!("expected format name:value,... (got {})", mapping_str)));
        }
